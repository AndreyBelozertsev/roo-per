...
//        }
    ...
    {
        $sqlContent = '';
        $sqlContent = "ALTER TABLE menu_node ADD COLUMN IF NOT EXISTS old_url VARCHAR(2000) DEFAULT NULL;"."\r\n";
        $sqlContent .= "ALTER TABLE menu_node ADD COLUMN IF NOT EXISTS old_domain VARCHAR(2000) DEFAULT NULL;"."\r\n";
        $sqlContent .= "ALTER TABLE menu_node ADD COLUMN IF NOT EXISTS old_redirect VARCHAR(2000) DEFAULT NULL;"."\r\n";

        $sqlContent .= "DROP INDEX IF EXISTS uniq_4d30180b989d9b62;"."\r\n";
...
ON CONFLICT(code) DO NOTHING;"."\r\n";
    ...
    VALUES('".$slug."', '".$row['URL']."', '".$row['DOMAIN']."', '".$row['REDIRECT_TO']."', ".$this->structureID.", '".$this->cleanString($row['NAME'])."', '".$beforeText."', ".$template.", ".(int)$row['ID'].", ".(int)$row['ORIG_ID'].", ".(int)$row['PARENT_ID'].", ".(int)$row['HIDE'].", ".(int)$row['DRAFT'].") ON CONFLICT(old_id) DO NOTHING;"."\r\n";
                        file_put_contents($this->rootDir . "web/oracle/{$code}/migrations/sql/migration.sql", $sqlContent, FILE_APPEND);
...
PERSON_DEPARTMENT pd LEFT JOIN POST po ON po.ID = pd.POST_ID LEFT JOIN PERSON p ON p.ID = pd.PERSON_ID WHERE pd.ID = ".$row['ORIG_ID'];
    ...
    VALUES('".$slug."', '".$row['URL']."', '".$row['DOMAIN']."', '".$row['REDIRECT_TO']."', ".$this->structureID.", '".$this->cleanString($row['NAME'])."', '".$beforeText."', ".$template.", ".(int)$row['ID'].", ".(int)$row['ORIG_ID'].", ".(int)$row['PARENT_ID'].", ".(int)$row['HIDE'].", ".(int)$row['DRAFT'].", 16) ON CONFLICT(old_id) DO NOTHING;"."\r\n";
                        file_put_contents($this->rootDir . "web/oracle/{$code}/migrations/sql/migration.sql", $sqlContent, FILE_APPEND);
                        file_put_contents($this->rootDir . "web/oracle/{$code}/sync/structure.txt", $row['ID'].' ', FILE_APPEND);
                        $writeMigration = true;
                    }elseif($row['TYPE'] == 15) {
...
PERSON p ON p.ID = pd.PERSON_ID WHERE d.PARENT_ID = '.$row['ORIG_ID'];
                        ...
                        while (($tbl = oci_fetch_assoc($podved)) != false) {
                            ...
                            $sqlContent .= "INSERT INTO menu_node(slug, old_url, old_domain, old_redirect, menu_id, title, before_text, structure_template_id, old_id, orig_id, old_parent_id, hide, draft)
    VALUES('".$slug."', '".$row['URL']."', '".$row['DOMAIN']."', '".$row['REDIRECT_TO']."', ".$this->structureID.", '".$this->cleanString($tbl['dep'])."', '".$beforeText."', ".$template.", ".(int)$tbl['ID'].", ".(int)$row['ORIG_ID'].", ".(int)$row['ID'].", 0, 0) ON CONFLICT(old_id) DO NOTHING;"."\r\n";
                            file_put_contents($this->rootDir . "web/oracle/{$code}/migrations/sql/migration.sql", $sqlContent, FILE_APPEND);
